...
from fastapi import APIRouter, UploadFile, File, HTTPException, BackgroundTasks, Request, Query
...
async def upload_excel(
    ...
    department: str = Query(default="", description="اسم القسم"),
):
    global _active_jobs
    ...
    if ext not in Config.ALLOWED_EXTENSIONS:
        raise HTTPException(status_code=400, detail=f"نوع الملف غير مدعوم. المسموح: xlsx, xls")
    ...
    background_tasks.add_task(
        process_excel_file, job_id, temp_path, department, client_ip, size
    )
...
# ─────────────────────────────────────────────
# GET /job/{job_id}  →  polling endpoint
# ─────────────────────────────────────────────
...
async def get_job_status(job_id: str):
    ...
    # Attach stats if done
    if job["status"] in [JobStatus.COMPLETED, JobStatus.PARTIAL_SUCCESS, JobStatus.FAILED]:
...
# ─────────────────────────────────────────────
# GET /result/{job_id}  →  index of all students
# ─────────────────────────────────────────────
...
async def get_result(job_id: str):
    ...
    return {
        ...
        "students": data.get("students", []),   # [{student_id, name, sheet_name}]
        "errors": data.get("errors", []),
    }


# ─────────────────────────────────────────────
# GET /result/{job_id}/student/{student_id}
# ─────────────────────────────────────────────
@router.get(
    "/result/{job_id}/student/{student_id}",
    summary="بيانات طالب واحد بالتفصيل",
)
async def get_student_detail(job_id: str, student_id: str):
    ...
    if not student_file.exists():
        raise HTTPException(status_code=404, detail=f"الطالب {student_id} غير موجود في هذه المهمة")
...
# ─────────────────────────────────────────────
# GET /result/{job_id}/students/batch
# Get multiple students at once (Flutter optimization)
# ─────────────────────────────────────────────
@router.get(
    "/result/{job_id}/students/batch",
    summary="جلب بيانات مجموعة طلاب دفعة واحدة",
)
...
# ─────────────────────────────────────────────
# DELETE /job/{job_id}  →  manual cleanup
# ─────────────────────────────────────────────
...
async def process_excel_file(
    ...
    try:
        ...
        # Run blocking parse in thread pool (don't block event loop)
        loop = asyncio.get_event_loop()
        students_data, parsing_errors = await loop.run_in_executor(
            None, _parse_excel_sync, file_path, department
        )
        ...
        status = (
            JobStatus.COMPLETED if not parsing_errors else JobStatus.PARTIAL_SUCCESS
        )
        ...
        job_manager.update_job(
            ...
            error_log=[str(e)],
            stats={"total_students": 0, "successful": 0, "failed": 0},
        )
...
def _parse_excel_sync(file_path: Path, department: str):
    """Sync wrapper for ExcelParser (runs in thread pool)"""
